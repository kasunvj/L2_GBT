			...
			case 'L':
				switch (page) {
					case 0://last charge[chargerID,LastCharge%,time,Cost,kwhRate]
						...
						/* Charger ID Char*/
						cidChar = Buffer.from((netDMGLeft.cidChar).toString('hex')
						dmgDataBuf = Buffer.concat([Buffer.from([0x14, 0x30]),cidChar], 4);
						objDMG.dmgCIDChangeMsg(dmgDataBuf);
			...
			case 'R':
				switch (page) {
					case 0://IDELING  PAGE[chargerID,LastCharge%,time,Cost,kwhRate]
						...
						/* Charger ID Char*/
						cidChar = Buffer.from((netDMGRight.cidChar).toString('hex')
						dmgDataBuf = Buffer.concat([Buffer.from([0x14, 0x40]),cidChar], 4);
						objDMG.dmgCIDChangeMsg(dmgDataBuf);
